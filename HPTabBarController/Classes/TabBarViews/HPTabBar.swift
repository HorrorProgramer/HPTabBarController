...
public enum HPTabBarItemPositioning : Int {
    case automatic
...
internal protocol HPTabBarDelegate: NSObjectProtocol {
...
open class HPTabBar: UITabBar {
    
    internal weak var customDelegate: HPTabBarDelegate?
    ...
    public var itemCustomPositioning: HPTabBarItemPositioning? {
        didSet {
    ...
    internal var containers = [HPTabBarItemContainer]()
    ...
    open var moreContentView: HPTabBarItemContentView? = HPTabBarItemMoreContentView.init() {
        didSet { self.reload() }
    }
    ...
    open override func beginCustomizingItems(_ items: [UITabBarItem]) {
        dPrint("beginCustomizingItems(_:) is unsupported in ESTabBar.")
        super.beginCustomizingItems(items)
    }
    
    open override func endCustomizing(animated: Bool) -> Bool {
        dPrint("endCustomizing(_:) is unsupported in ESTabBar.")
...
internal extension HPTabBar /* Layout */ {
...
internal extension HPTabBar /* Actions */ {
    
    internal func isMoreItem(_ index: Int) -> Bool {
        return HPTabBarController.isShowingMore(tabBarController) && (index == (items?.count ?? 0) - 1)
    }
    ...
    internal func reload() {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            let container = HPTabBarItemContainer.init(self, tag: 1000 + idx)
            self.addSubview(container)
            self.containers.append(container)
            
            if let item = item as? HPTabBarItem, let contentView = item.contentView {
    ...
    @objc internal func highlightAction(_ sender: AnyObject?) {
        guard let container = sender as? HPTabBarItemContainer else {
            return
        }
        ...
        if let item = item as? HPTabBarItem {
    ...
    @objc internal func select(itemAtIndex idx: Int, animated: Bool) {
        ...
        if currentIndex != newIndex {
            if currentIndex != -1 && currentIndex < items?.count ?? 0{
                if let currentItem = items?[currentIndex] as? HPTabBarItem {
    ...
    internal func updateAccessibilityLabels() {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            ...
            if self.isMoreItem(idx) {
                accessibilityTitle = NSLocalizedString("More_TabBarItem", bundle: Bundle(for:HPTabBarController.self), comment: "")
            }
            ...
            let formatString = NSLocalizedString(item == selectedItem ? "TabBarItem_Selected_AccessibilityLabel" : "TabBarItem_AccessibilityLabel",
                                                 bundle: Bundle(for: HPTabBarController.self),
                                                 comment: "")
